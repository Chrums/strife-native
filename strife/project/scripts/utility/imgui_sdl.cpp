...
struct Color {
    ...
    uint32_t ToInt() const {
        return ((static_cast<int>(R * 255) & 0xff) << 0) | ((static_cast<int>(G * 255) & 0xff) << 8) | ((static_cast<int>(B * 255) & 0xff) << 16) | ((static_cast<int>(A * 255) & 0xff) << 24);
    }

    void UseAsDrawColor(SDL_Renderer* renderer) const {
        SDL_SetRenderDrawColor(renderer, static_cast<uint8_t>(R * 255), static_cast<uint8_t>(G * 255), static_cast<uint8_t>(B * 255), static_cast<uint8_t>(A * 255));
    }
};
...
struct Device {
    ...
    Device(SDL_Renderer* renderer, Texture* fontTexture)
        : Renderer(renderer)
...
struct Rect {
    ...
    static Rect CalculateBoundingBox(const ImDrawVert& v0, const ImDrawVert& v1, const ImDrawVert& v2) {
        return Rect{std::min({v0.pos.x, v1.pos.x, v2.pos.x}), std::min({v0.pos.y, v1.pos.y, v2.pos.y}), std::max({v0.pos.x, v1.pos.x, v2.pos.x}), std::max({v0.pos.y, v1.pos.y, v2.pos.y}),
                    std::min({v0.uv.x, v1.uv.x, v2.uv.x}),    std::min({v0.uv.y, v1.uv.y, v2.uv.y}),    std::max({v0.uv.x, v1.uv.x, v2.uv.x}),    std::max({v0.uv.y, v1.uv.y, v2.uv.y})};
    }
};
...
void DrawTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const Device::Texture* texture) {
    ...
    const auto key = std::make_tuple(std::make_tuple(static_cast<int>(std::round(v1.pos.x)) - renderInfo.MinX, static_cast<int>(std::round(v1.pos.y)) - renderInfo.MinY, v1.uv.x, v1.uv.y, v1.col),
    ...
    DrawTriangleWithColorFunction(renderInfo,
                                  [&](float x, float y) {
...
void DrawUniformColorTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3) {
    ...
    const auto key = std::make_tuple(v1.col, static_cast<int>(std::round(v1.pos.x)) - renderInfo.MinX, static_cast<int>(std::round(v1.pos.y)) - renderInfo.MinY,
...
void DrawRectangle(const Rect& bounding, SDL_Texture* texture, const Color& color, bool doHorizontalFlip, bool doVerticalFlip) {
    ...
    const SDL_Rect destination = {static_cast<int>(bounding.MinX), static_cast<int>(bounding.MinY), static_cast<int>(bounding.MaxX - bounding.MinX), static_cast<int>(bounding.MaxY - bounding.MinY)};
    ...
    if (bounding.UsesOnlyColor()) {
        ...
        const SDL_Rect source = {static_cast<int>(bounding.MinU * textureWidth), static_cast<int>(bounding.MinV * textureHeight), static_cast<int>((bounding.MaxU - bounding.MinU) * textureWidth),
    ...
    void Render(ImDrawData* drawData) {
        ...
        for (int n = 0; n < drawData->CmdListsCount; n++) {
            ...
            for (int cmd_i = 0; cmd_i < commandList->CmdBuffer.Size; cmd_i++) {
                ...
                const Device::ClipRect clipRect = {static_cast<int>(drawCommand->ClipRect.x), static_cast<int>(drawCommand->ClipRect.y),
                                                   static_cast<int>(drawCommand->ClipRect.z - drawCommand->ClipRect.x), static_cast<int>(drawCommand->ClipRect.w - drawCommand->ClipRect.y)};
                ...
                if (drawCommand->UserCallback) {
                    ...
                    for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3) {
                        ...
                        if (i + 6 <= drawCommand->ElemCount) {
                            ...
                            if (isUniformColor && bounding.IsOnExtreme(v0.pos) && bounding.IsOnExtreme(v1.pos) && bounding.IsOnExtreme(v2.pos) && bounding.IsOnExtreme(v3.pos) &&
